...
class Common extends CI_Controller
{
        ...
        switch ($data->infoType) {
            case "banner":
                ...
                $info = array(
                    ...
                    "thumbnail" => strstr($data->thumbnail, 'http') ? $data->thumbnail : SERVER_IP .
                        ($data->thumbnail),
            ...
            case "businessdistrict":
                ...
                $object['business'] = array(
                    ...
                    "businessLogo" => strstr($data->thumbnail, 'http') ? $data->thumbnail :
                        SERVER_IP . ($data->thumbnail),
            ...
            case "wordsort":
                ...
                $object['wordSort'] = array("wordSortName" => $data->word_sort_name);
            ...
            case "worditem":
                ...
                $object['wordItem'] = array("wordItemSortId" => $data->word_item_sort_id,
            ...
            case "worditemadd":
                $object['wordItem'] = array();
                $object['wordItem'] = array("wordItemShopId" => $_SESSION['shopid'],
                        "wordItemName" => $data->word_item_name);
                $wordItem = json_encode($object);
                $url = API_IP . 'AEWebApp/userShop/insertWordItem';
                $partam = 'wordItem=' . $wordItem;
            ...
            case "catefeature":
                ...
                // }
                /*判断添加的分类特征是否存在*/
                if ($this->getdata->isExist('categoryfeature', array('feature_name' => $data->
                        feature_name))) {
                    echo json_encode(array("result" => "failed", "message" => "该分类特征已存在！"));
                    return false;
                }
                $object = array();
                $object = array("feature_name" => $data->feature_name, "feature_addtime" => $time);
            ...
            case "catefeatureval":
                ...
                $time = date("Y-m-d H:i:s");
                if ($this->getdata->isExist('categoryeigenvalue', array('eigen_name' => $data->
                        eigenvalue_name))) {
            ...
            case "reminder":
                ...
                $object['reminder'] = array("msgContent" => $data->msg_content, "msgStatus" => $data->
                        msg_status);
            ...
            case "activity":
                ...
                $object['activity'] = array(
                    ...
                    "content" => $data->content,
                    "thumbnail1" => strstr($data->thumbnail1, 'http') ? $data->thumbnail1 :
                        SERVER_IP . ($data->thumbnail1),
    ...
    public function modifyInfo()
    {
        ...
        switch ($data->infoType) {
            ...
            case "password":
                ...
                $info = $this->getdata->getContentAdvance('admin', array('username' => $data->
                        username));
            ...
            case "annual":
                ...
                $moneyresult = $this->dbHandler->updateData(array(
                    'table' => $monrytable,
                    'where' => $monrywhere,
                    'data' => $money));
                if ($moneyresult == 1) {
            ...
            case "buyer":
                ...
                if (isset($data->photo)) {
                    $info['photo'] = strstr($data->photo, 'http') ? $data->photo : SERVER_IP . ($data->
                        photo);
                }
            ...
            case "activity":
                ...
                if (isset($data->thumbnail1)) {
                    $info['thumbnail1'] = strstr($data->thumbnail1, 'http') ? $data->thumbnail1 :
                        SERVER_IP . ($data->thumbnail1);
                }
            ...
            case "product":
                ...
                if (!empty($data->pic1)) {
                    $info['pic1'] = strstr($data->pic1, 'http') ? $data->pic1 : SERVER_IP . ($data->
                        pic1);
                } else {
                    $info['pic1'] = '';
                }
            ...
            case "coupon":
                ...
                $partam = 'coupon=' . json_encode($adSpot);
            ...
            case "word":
                $table = 'wordlist';
                $where = array('word_id' => $data->id);
                $info = array('word_edittime' => date("Y-m-d H:i:s"));
                $info['audit_status'] = 1;
                $info['word_status'] = 0;
                $result = $this->dbHandler->updateData(array(
                    'table' => $table,
                    'where' => $where,
                    'data' => $info));
                die(json_encode(array('result' => 'dasda', 'message' => $result)));
                //$info = array('couponId' => $data->id);
                //                $info['auditStatus'] = $data->status;
                //                $info['couponStatus'] = '0';
                //                $adSpot = new stdClass;
            ...
            case "businessdistrict":
                ...
                if (isset($data->thumbnail1)) {
                    $info['businessLogo'] = strstr($data->thumbnail1, 'http') ? $data->thumbnail1 :
                        SERVER_IP . ($data->thumbnail1);
                }
            ...
            case "usershop":
                ...
                if (isset($data->thumbnail)) {
                    $info['shopLogo'] = strstr($data->thumbnail, 'http') ? $data->thumbnail :
                        SERVER_IP . ($data->thumbnail);
                }
                if (isset($data->thumbnail1)) {
                    $info['shopTopPic1'] = strstr($data->thumbnail1, 'http') ? $data->thumbnail1 :
                        SERVER_IP . ($data->thumbnail1);
                }
                if (isset($data->thumbnail2)) {
                    $info['shopTopPic2'] = strstr($data->thumbnail2, 'http') ? $data->thumbnail2 :
                        SERVER_IP . ($data->thumbnail2);
                }
                if (isset($data->thumbnail3)) {
                    $info['shopTopPic3'] = strstr($data->thumbnail3, 'http') ? $data->thumbnail3 :
                        SERVER_IP . ($data->thumbnail3);
                }
                if (isset($data->thumbnail4)) {
                    $info['shopTopPic4'] = strstr($data->thumbnail4, 'http') ? $data->thumbnail4 :
                        SERVER_IP . ($data->thumbnail4);
                }
                if (isset($data->thumbnail5)) {
                    $info['shopTopPic5'] = strstr($data->thumbnail5, 'http') ? $data->thumbnail5 :
                        SERVER_IP . ($data->thumbnail5);
                }
        ...
        switch ($data->infoType) {
            case "activity":
                ...
                if ($data->status == 1) {
                    ...
                    $infos = array("status" => '0');
                    $update = $this->dbHandler->updateData(array(
                        'table' => $table,
                        'where' => $wheres,
                        'data' => $infos));
                    $info['status'] = 1;
    ...
    public function deleteBulkInfo()
    {
        ...
        foreach ($data->idArray as $value) {
            $result = $this->dbHandler->deleteData(array('table' => $table, 'where' => array
                    ($where => $value)));
        }
        echo json_encode(array("result" => "success", "message" => "信息删除成功"));
    }
    ...
    public function addCategory()
    {
        ...
        switch ($data->infoType) {
            case "category":
                if ($this->getdata->isExist('category', array('name' => $data->name, 'twoid' =>
                        '1'))) {
    ...
    public function getInfo()
    {
        ...
        switch ($data->infoType) {
            ...
            case 'shopcategory':
                $categoryid = $data->id;
                $url = API_IP . 'AEWebApp/userShop/queryGoodsListByCategoryId?categoryShopId=' .
                    $categoryid;
                $header = array();
                $param = array();
            ...
            case 'supermarket':
                ...
                break;
            case 'categories':
                $result = $this->getdata->getCategory(array('result' => 'data', 'sid' => $data->
                        id));
                break;
            case 'supcategories':
                $result = $this->getdata->getSupCategory(array('result' => 'data', 'sid' => $data->
                        id));
                break;
        }
