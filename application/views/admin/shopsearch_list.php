...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 搜索审核员管理 <span class="c-gray en">&gt;</span> 店铺关键字管理 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				<td><input type="checkbox" value="<?php echo $buyer->shop_id;?>" name="id"></td>

				<td>
					<?php if(!empty($buyer->shop_name)):?>
						<?php echo $buyer->shop_name;?>
					<?php else:?>
				    	暂无店铺名称
				    <?php endif;?>
				</td>
				<td>
					<?php if(!empty($buyer->shop_branch_name)):?>
						<?php echo $buyer->shop_branch_name;?>
					<?php else:?>
				    	暂无分店名称
				    <?php endif;?>
				</td>
                ...
                <td>
					<?php if(!empty($buyer->username->user_nickname)):?>
						<?php echo $buyer->username->user_nickname;?>
				    <?php endif;?>
				</td>
                
				<td>
					<?php if(!empty($buyer->shop_keywords)):?>
						<?php echo $buyer->shop_keywords;?>
					<?php else:?>
				    	暂无关键字
				    <?php endif;?>
				</td>
				...
				<td>
					<?php if(!empty($buyer->shop_addtime)):?>
						<?php echo $buyer->shop_addtime;?>
					<?php else:?>
						暂无注册时间
				...
				<td>
					<?php if(!empty($buyer->shop_edittime)):?>
						<?php echo $buyer->shop_edittime;?>
					<?php else:?>
						暂无更新时间
					<?php endif;?>
				</td>
